...
int main(int argc, char *argv[])
{
    ...
    if(NULL != fileName)
    {
        ...
        if (NULL == (sessionHandle =
            pcap_open_offline(fileName, errorBuffer)))
        {
        ...
        if (NULL == ( sessionHandle = 
            pcap_open_live(deviceEther, BUFSIZ, 1, MS_DELAY,  errorBuffer)))
        {
    ...
    /* Compile The Filter So That The Content Of A Specific Port May Be Sniffed Only. */
    /* if(-1 == pcap_compile(sessionHandle, &filter, filter_app, 0, 0xFFFFFF00)) */
    if(-1 == pcap_compile(sessionHandle, &filter, filter_app, 0, ipOfDeviceEther)) 
    {
...
void sniff_packets(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
	...
	static const char *virusString ="X5O!P%@AP[4\\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*";
	char testBuffer[100]; 
	...
	#ifdef DEBUG
	if('\0' == paybuf[size_payload]) printf("YEEEEEEEEEEEEEEEHAAAAAAAAAWWWWWWWWWWWW!!!!!!!!!!!!!!!!!!!!!!\n\n");
	#endif
			...
			return;
			 ...
			 int j=0;
				...
				while('\0' != paybuf[i])
				{
					...
					if( paybuf[i] == characterRef[j] || paybuf[i] == (characterRef[j] + ('e'-'E'))) 
					{
						if(0==j) 
						{
							...
							if(0 == strcmp(testBuffer, attachField) || testBuffer[32] == attachField[32])
							{
								#ifdef DEBUG	
						...
						if(2==j) 
						{
							...
							if(0 == strcmp(testBuffer, virusString))
							{
								...
								printf("\x07Virus reported! \nFrom: %s\nTo: %s\nFi"
										...
										"Proceeding to clean... \n\n"
										"CLEANED!\n\n", 
										email_data.from, email_data.to, 
										email_data.com, email_data.virus);
